...
- [5. 次の一手](#5-次の一手)
  ...
  - [5.2. ArduPilot 公式 Extension を読む](#52-ardupilot-公式-extension-を読む)
...
ドローン Web アプリ開発編の Web アプリ生成（第2部）と Extension 化（第4部）は、ターミナルで動く **AI コーディングエージェント** で進めます。下記のいずれか（CLI 版。IDE 拡張ではありません）を **WSL（Ubuntu）にインストール**してください。
...
> **演習では完成版を使います（3分クッキング）。** アプリの生成（2.1）は宿題です。演習時間中は完成・素版 **`webapp-blueos/drone-web-app/`** をワークフォルダにコピーして、ローカル動作確認（2.2）を行います。
...
```bash
# ② 完成版をワークフォルダにコピー（演習用。宿題で自分のアプリを生成した場合は不要）
cp -r ~/GitHub/droneschool/webapp-blueos/drone-web-app ~/GitHub/droneschool/workshop/<term_no>/<fname-lname>/
```

```bash
# ③ アプリ起動（接続先をローカル SITL に上書き）
cd ~/GitHub/droneschool/workshop/<term_no>/<fname-lname>/drone-web-app/backend
...
<img src="media/blueos-mavlink-endpoints-010.png" width="340" alt="MAVLink Endpoints 画面">
...
第4部の流れは「**作る（WSL／4.1）→ 配る（4.2 前半・Docker Hub へ push/pull）→ 動かす（4.2 後半・BlueOS 上で MAVLink Server に接続）**」の3段階です。
...
> **演習では完成版を使います（3分クッキング）。** `/dronify-blueos` の実行は宿題です。演習時間中は、すでに要件適用済みの完成・BlueOS版 **`webapp-blueos/drone-web-app-blueos/`** をワークフォルダにコピーして、コンテナのビルド・起動を確認します（下記「ローカル確認」）。
...
**BlueOS**＝BlueOS Extension である以上どのアプリでも必須の要件、**Webアプリ**＝このアプリの実装（WebSocket・地図・MAVLink 処理）に起因して個別に必要な対策です。

| # | 要件 | 由来 | なぜ必要か |
|---|------|------|-----------|
| ① | **permissions LABEL（bridge + ポート固定）** | BlueOS | Kraken は LABEL を Docker API 設定としてそのまま使う。`avoid_iframes` で `http://<IP>:9999/` を直接開くためポートは固定（`HostPort:"9999"`） |
| ②a | **`/register_service` を実装** | BlueOS | Helper はこの JSON を見て左メニューを生成する（全 Extension 共通の必須要件） |
| ②b | **`avoid_iframes: true`** | Webアプリ | このアプリは WebSocket を使うが、nginx の自動ルートは WS 非対応のため、iframe 経由ではなくポートへ直接アクセスさせる必要がある |
| ③ | **`GET /` が 200 を返す** | BlueOS | Helper は `/` で生存確認する。200 以外だと `/register_service` を呼ばずメニューに出ない |
| ④a | **`host.docker.internal` で MAVLink Server に接続** | BlueOS | コンテナからホスト側の MAVLink Server（内部 14550）へ到達するための接続先。BlueOS のコンテナ実行環境に起因 |
| ④b | **`mode_mapping_byname` での明示マップ・自機のみ受信** | Webアプリ | 機体種別ごとのモード番号のずれ、複数 GCS 接続時の混信による mode/armed の点滅を防ぐための、このアプリ側の実装対策 |
| ⑤ | **Leaflet をローカル同梱** | Webアプリ | このアプリは Leaflet を CDN から読み込む実装のため。BlueOS ホットスポット接続中はネットに出られず CDN 不達になる（`L is not defined`） |
...
演習では、すでに要件が適用済みの完成・BlueOS版 **`webapp-blueos/drone-web-app-blueos/`** をワークフォルダにコピーして、コンテナ起動を確認します（スキル適用は宿題）。
...
3. 以下を入力する
   ...
   <img src="media/blueos-extensions-010.png" width="440" alt="Create Extension ダイアログ">
...
| TAKEOFF で AUTOTUNE 失敗 | Plane のモードマップを誤適用 | `mode_mapping_byname` で機体タイプから生成（4.1 ④b） |
| ARM/モード表示が点滅 | GCS の HEARTBEAT も拾っている | 受信を自機（target）に限定（4.1 ④b） |
...
> 他の Extension（`blueos-visual-follow`、`blueos-camera-gimbal-control`、`blueos-cloud-upload` など）も参考になります。一覧: <https://github.com/rmackay9?tab=repositories&q=blueos>
