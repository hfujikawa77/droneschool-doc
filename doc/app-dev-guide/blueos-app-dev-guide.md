...
1. GitHub 上で自分のフォークの `master` を **Sync fork**
2. ローカルで master を更新し、自分のワークブランチへ取り込む：
   ...
   git checkout master
   git pull
   git checkout <term_no>_<fname-lname>   # 自分のワークブランチ
   git merge master
   ```
...
応用編の Web アプリ生成（第2部）と Extension 化（第4部）は、ターミナルで動く **AI コーディングエージェント** で進めます。

- **Claude Code・Codex を推奨**します。**GitHub Copilot CLI** でも実施可能です（いずれも CLI 版。IDE 拡張ではありません）。
...
mkdir -p ~/GitHub/droneschool/workshop/<term_no>/<fname-lname>
cp ~/GitHub/droneschool/webapp-blueos/build-prompt.md \
   ~/GitHub/droneschool/workshop/<term_no>/<fname-lname>/
```
...
> 🖼️ **TODO（画像）**: `Vehicle Setup → MAVLink Endpoints` 画面（`UDP Server 0.0.0.0:14550` が存在する状態）
