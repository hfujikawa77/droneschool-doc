...
本手順書では、以下の2種類のシミュレータ起動方法を扱います。起動方法によって、Pythonスクリプトからの接続先IPアドレスが異なります。

| 種別 | 起動方法 | 動作環境 | 接続先IPアドレス |
|------|----------|----------|-----------------|
| **MP SITL** | [2.2節](#22-シミュレータmission-plannerの起動)（Mission Plannerから起動） | Windows上 | `＜PCのIPアドレス＞`（後述） |
| **WSL SITL** | [8.1節](#81-シミュレータ動作確認)（`sim_vehicle.py`で起動） | WSL（Ubuntu）上 | `127.0.0.1` |

**MP SITLを使用する場合：** PowerShellで`ipconfig`コマンドを実行し、PCのIPv4アドレスを調べておきます。  
...
ソースコード4～5行目 の接続先を、使用するシミュレータに合わせて修正します。

| 種別 | 接続先の設定値 |
|------|--------------|
| MP SITL | `tcp:＜PCのIPアドレス＞:5762` |
| WSL SITL | `tcp:127.0.0.1:5762` |
