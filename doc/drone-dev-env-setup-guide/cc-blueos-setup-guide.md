...
- [4. 疎通確認](#4-疎通確認)
  ...
  - [4.3. WSL上のPythonアプリからの接続確認](#43-wsl上のpythonアプリからの接続確認)
  - [4.4. 飛行動作の確認（任意）](#44-飛行動作の確認任意)
- [5. Appendix](#5-appendix)
  - [5.1. BlueOS](#51-blueos)
  - [5.2. ArduPilot SITL](#52-ardupilot-sitl)
  - [5.3. Mission Planner](#53-mission-planner)
  - [5.4. MAVLink-router](#54-mavlink-router)
  - [5.5. Pymavlink](#55-pymavlink)
...
│   開発アプリ（pymavlink）◀─┼─TCP:5777──│   ├─（各クライアントへ配信）  │
│                          │             │   └─ Extension               │
...
## 1.2. 本書の位置づけ
本書のゴールは、**WSL上のSITLをBlueOSに機体として接続し、PC側の各クライアント（Mission Planner、Pythonアプリ）からBlueOS経由で機体に接続できること**を確認するところまでです。
...
| Pythonアプリ | TCP 192.168.42.1:5777 | MAVLink RouterのTCPサーバポート |
...
## 4.3. WSL上のPythonアプリからの接続確認
アプリ開発で使用するWSL（Ubuntu）上のpymavlinkから、**BlueOS経由で**テレメトリを受信できることを確認します。  
※ SITLはWSL上で動いているため直接接続もできますが、ここでは開発アプリと同じ経路（BlueOSのMAVLink Router経由）で受信できることを確認します。

VS CodeでWSLに接続し、ターミナルを開きます（SITLのターミナルとは別に開きます）。  
下記の内容で `check_blueos.py` を作成します。

```python
from pymavlink import mavutil

# BlueOS（ラズパイ）のMAVLink RouterにTCPで接続
master = mavutil.mavlink_connection("tcp:192.168.42.1:5777")
...
```bash
python3 check_blueos.py
```

HEARTBEATの受信に続いて、ATTITUDEメッセージが5件表示されれば疎通確認完了です。

```
HEARTBEAT受信: system=1 component=1
ATTITUDE {time_boot_ms : 123456, roll : 0.001, pitch : -0.002, ...}
（以下略）
```

※ Mission Planner（UDP 14550）と同時に接続したままで構いません。経路が異なるため競合しません。
<!-- TODO: TCP 5777（MAVLink Routerの動的TCPサーバ）経由の接続は要実機確認。不可の場合はMAVLink EndpointsでUDP Serverエンドポイントを追加する方式に差し替える -->
...
以上で環境構築は完了です。アプリ開発の演習は別冊の**BlueOSアプリ開発ガイド**に進んでください。
